...
class FloatTable {
  ...
  int columnCount;
  float[][] data;
  //String[] rowNames;   // rowNames not required anymore
  String[] columnNames;
  ...
  FloatTable(String filename) {
    ...
    String[] columns = splitTokens(rows[0]);
    
        
    //columnNames = subset(columns, 1); // upper-left corner ignored
    columnNames = subset(columns, 0); // No need to ignore
    
    scrubQuotes(columnNames);
    columnCount = columnNames.length;

    //rowNames = new String[rows.length-1];   // Rowname not required
    ...
    for (int i = 1; i < rows.length; i++) {
      ...
      String[] pieces = splitTokens(rows[i]); // on space or newline
      scrubQuotes(pieces);
      
      // copy row title
      //rowNames[rowCount] = pieces[0];    // no title now
      
      // copy data into the table starting at pieces[1]
      //data[rowCount] = parseFloat(subset(pieces, 1));

      // copy data into the table starting at pieces[0] now
